...
<gate_expected_result>Os 8 filtros selecionam pelo menos um teste e passam sem falhas; os testes unitários comprovam transições, outcomes, guards, correlação e falha best-effort; os testes de integração comprovam consumo real, persistência, payload/headers CloudEvents e ausência de publicação duplicada; `dotnet build` e format scoped do gate ficam verdes</gate_expected_result>
<static_evidence>N/A — behavioral</static_evidence>
<vertical_slice>Ao receber um `payment.payment_authorized` ou `payment.payment_rejected` com `correlationId` válido e conhecido, Booking conclui a Reservation Saga em `confirmada`/`Authorized` ou `cancelada`/`Rejected` com o motivo de negócio, persiste o estado e publica exatamente o evento final correspondente. Mensagens com correlação ausente/inválida/desconhecida, duplicadas, tardias ou conflitantes não alteram uma Reservation terminal nem publicam novamente; se o publish final falhar, o estado persistido permanece terminal e a falha é apenas logada, sem retry ou Outbox nesta fase.</vertical_slice>
</task_context>
...
`IReservationRepository.UpdateAsync` devem ser reutilizados sem reimplementação. Não há migration
nova nesta feature.
...
- **Resultado esperado — autorização:** Reservation `Solicitada` → `Confirmada`, Saga `PaymentPending` → `Authorized`, persistência via `UpdateAsync` e um evento `booking.reservation_confirmed` com payload/headers do contrato.
- **Resultado esperado — rejeição:** Reservation `Solicitada` → `Cancelada`, Saga `PaymentPending` → `Rejected`, `CancellationReason = "Pagamento rejeitado pela simulação de Payment."`, persistência via `UpdateAsync` e um evento `booking.reservation_cancelled` com payload/headers do contrato.
...
- `Reservation.Confirm()` só aceita `Status == Solicitada`, atribui `Confirmada` e chama `Saga.MarkAuthorized()`.
- `Reservation.Cancel(string cancellationReason)` só aceita `Status == Solicitada`, atribui `Cancelada` e chama `Saga.MarkRejected(cancellationReason)`.
- A guarda privada de domínio lança `InvalidOperationException` para qualquer estado não solicitado; o handler deve verificar o estado antes, para que duplicidade/tardio seja um outcome normal e não exceção de controle.
...
- **Modificar:**
  - `services/booking/src/3-Domain/LocalizeStay.Booking.Domain/Reservations/Reservation.cs` — adicionar transições e guarda de invariante.
...
4. `ConfirmReservationCommandHandler` usa `GetByCorrelationIdAsync`. `null` retorna `NotCorrelatable` após log; estado diferente de `Solicitada` retorna `AlreadyTerminal` após log; somente estado solicitado chama `reservation.Confirm()`.
5. O handler chama `UpdateAsync` antes de publicar. O publisher envia `booking.reservation_confirmed` com `confirmedAt = DateTimeOffset.UtcNow`, campos contratuais, CloudEvent type versionado e os dois headers de correlação.
...
3. `CancelReservationCommandHandler` repete a busca/decisão de correlação e estado terminal, chama `reservation.Cancel(reason)` somente em `Solicitada`, persiste via `UpdateAsync` e publica `booking.reservation_cancelled` best-effort.
4. O payload de cancelamento usa `cancelledAt = DateTimeOffset.UtcNow`, `reservation.Saga.CancellationReason` e os campos mínimos do contrato; nunca propaga eventual motivo técnico de Payment.
...
- `ReservationTests`: confirmação/cancelamento desde `Solicitada`, marcação da Saga e `InvalidOperationException` desde `Confirmada`/`Cancelada`.
...
- Integração: criar Reservation por F01 com `FakeCatalogServerFactory`, publicar o envelope Payment em exchange real, aguardar com timeout curto o banco, conferir estado/Saga e ler uma fila temporária nas exchanges finais. Validar `type`, `data`, headers, campos mínimos e ausência de segunda mensagem após duplicado/tardio/conflitante.
...
- **Decisões fechadas:** uma única V-01; nomes dos commands/outcomes; busca por `Saga.CorrelationId` com `FirstOrDefaultAsync`; transições e guards de domínio; texto fixo de DP-01; exchanges/filas/routing keys/CloudEvent types; payload mínimo em `camelCase`; headers de correlação; persistir antes de publicar; publish best-effort sem retry/Outbox/dedup; `AddRmqTopicConsumer`; sem endpoint/frontend/migration, pacote ou versão nova. A referência de `AwesomeAssertions` é aditiva no projeto de testes e usa a versão central existente.
- **Limites de decisão do implementer:** nomes de métodos privados, organização interna dos testes e helpers de fixture, detalhes de `TopicSubscriptionOptions` equivalentes ao contrato, timeout curto de polling e timestamp de migration inexistente. Nenhuma dessas escolhas pode alterar o contrato ou criar tecnologia nova.
...
- **Artefatos exigidos pelo gate:** os 6 arquivos unitários novos e 2 arquivos de integração são criados nesta task; `ReservationTests`/`ReservationSagaTests` são modificados nesta task; fixtures, solution e biblioteca são preexistentes.
- **Dependências futuras:** Nenhuma. F05 e Catalog/Notification só consomem os eventos depois e não são necessários para compilar ou validar esta task.
...
- [ ] `ReservationTests` passa e comprova `Confirm`/`Cancel` desde `Solicitada`, marcação da Saga e guardas contra estados terminais.
...
- [ ] `PaymentAuthorizedConsumptionTests` passa contra Postgres/RabbitMQ Testcontainers: autorização correlacionada persiste `confirmada`/`Authorized`, publica exatamente um `booking.reservation_confirmed` fiel ao contrato, ignora duplicado/tardio e não correlacionável.
- [ ] `PaymentRejectedConsumptionTests` passa contra Postgres/RabbitMQ Testcontainers: rejeição correlacionada persiste `cancelada`/`Rejected` + motivo, publica exatamente um `booking.reservation_cancelled` fiel ao contrato, ignora duplicado/tardio/conflitante e não correlacionável.
- [ ] Os payloads de integração contêm somente os campos aprovados, `camelCase`, timestamps UTC, `correlationId`/`causationId` coerentes e headers `x-correlation-id`/`x-causation-id` esperados.
...
- [ ] Falha do publisher final é logada sem rethrow, sem nova tentativa automática e sem alterar o estado terminal já salvo.
- [ ] Nenhuma migration, pacote/versão nova, endpoint HTTP, frontend, Outbox, dedup store ou consumidor de Catalog/Notification é adicionado por esta task; a referência de `AwesomeAssertions` usa a versão já centralizada.
