...
Aprovado pelo autor sem novas ressalvas nesta revisão. As pendências listadas em "Questões em Aberto"
são confirmações factuais a fazer durante a implementação (versões, caminhos materializados pela
Fundação/Cadastro de Property), não decisões em aberto — nenhuma bloqueia esta aprovação.
...
Nenhuma questão bloqueia esta aprovação. Na implementação, confirmar apenas fatos que ainda não
existem no worktree:
...
1. **Implementação:** usar `tsg-flow-task-creator` referenciando este `frontend-techspec.md` junto com
   `prd.md`, `api-contract.yaml` e `techspec.md`, reconciliando as tasks backend preexistentes da
