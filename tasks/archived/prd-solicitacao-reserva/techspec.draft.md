...
- **A fundação técnica da Fase 0 (`tasks/prd-fundacao-fase0`) precisa estar executada para o serviço
  ...
  (`db/bootstrap`), e o vhost `/localize-stay` + `Rmq.CloudEvents` configurados (V-03 da fundação). No
  momento desta TechSpec, nenhum desses artefatos existe ainda no repositório — é uma dependência
  externa a este plano, não recriada aqui.
...
- [ ] **Conflito com o API Contract:** `api-contract.yaml` declara `servers.url` de desenvolvimento
  para Booking como `http://localhost:5000/v1` e para a dependência de Catalog como
  `http://localhost:5010/v1`. A TechSpec/tasks já aprovadas da fundação (`tasks/prd-fundacao-fase0`)
  fixam Booking em `:5102` e Catalog em `:5101`. Esta TechSpec resolve o conflito usando os valores já
  aprovados (`5101`/`5102`) na configuração real (`CatalogClient:BaseUrl`, launch profile de Booking);
  os `servers.url` do `api-contract.yaml` deveriam ser corrigidos para consistência num próximo ajuste
  do contrato — não bloqueia esta TechSpec porque não afeta schema, comportamento ou dado, só a URL
  ilustrativa de desenvolvimento no documento OpenAPI.
...
1. **Resolver Questões em Aberto** (principalmente a divergência de porta de dev entre `api-contract.yaml`
   e a fundação já aprovada) — proposta de resolução já registrada acima, pendente de confirmação.
2. **Aprovação:** promover para `techspec.md` com status `Aprovado` após confirmação.
3. **Implementação:** usar `tsg-flow-task-creator` referenciando esta TechSpec — mas antes, garantir
   que as tasks de `tasks/prd-fundacao-fase0` relevantes a Booking (schema/role, solution skeleton,
   vhost RabbitMQ) já foram executadas, já que esta TechSpec assume esses artefatos como pré-requisito
   e não os recria.
