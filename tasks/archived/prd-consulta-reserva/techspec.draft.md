...
>
> Três decisões desta revisão são materiais (mudam dado/arquitetura) e aguardam confirmação do
> autor antes da promoção a `Aprovado` — ver "Questões em Aberto": (1) completar o vocabulário de
> `SagaState`/`CancellationReason` no schema de Booking antes de F03/F04 existirem; (2) seeding via
> SQL direto nos testes de integração para simular estados que nenhum handler real ainda produz;
> (3) ordem de merge — `main` ainda não contém o código de F01 (está apenas na branch
> `feature/prd-solicitacao-reserva`), pré-requisito desta feature.
...
- **`main` ainda não contém o código de F01** (`Reservation`, `ReservationSaga`,
  `IReservationRepository`, `Dispatcher`, `GlobalExceptionHandler`, `CustomWebApplicationFactory`
  etc.) — todo esse código existe apenas na branch `feature/prd-solicitacao-reserva`, cuja TechSpec
  está `Aprovado` mas ainda não foi mesclada. Esta TechSpec assume que F01 estará mesclado em `main`
  antes (ou como parte da mesma entrega) desta feature ser implementada — precisa de confirmação do
  autor sobre a ordem/momento do merge antes de gerar as tasks.
...
- **Decisão:** Completar `SagaState` (`Authorized`, `Rejected`) e adicionar
...
- **Decisão:** os testes de integração usam `UPDATE` SQL direto (via `ExecuteSqlInterpolated`) para
  colocar uma `Reservation`/`ReservationSaga` nos estados `confirmada`/`cancelada` e
  `autorizado`/`rejeitado`, em vez de qualquer método de domínio.
...
- [ ] **Ordem de merge (bloqueante):** `main` ainda não contém o código de F01 (está só na branch
  `feature/prd-solicitacao-reserva`). Esta TechSpec assume F01 mesclado antes/junto da implementação
  desta feature. Confirmar com o autor antes de gerar as tasks.
- [ ] **Completar `SagaState`/`CancellationReason` nesta feature (em vez de esperar F04):** decisão
  proposta acima, apoiada nas Métricas de Sucesso do PRD; precisa de confirmação explícita do autor
  por mudar o schema compartilhado do domínio Booking antes de F03/F04 existirem.
- [ ] **Seed via SQL direto nos testes de integração:** decisão proposta acima; precisa de
  confirmação porque introduz uma dívida técnica documentada (helpers a revisar quando F04 existir).
...
1. **Aprovação:** resolver as três Questões em Aberto marcadas como materiais (ordem de merge de F01,
   extensão antecipada de schema, seed via SQL em teste) antes de promover este documento a
   `techspec.md` com status `Aprovado`.
2. **Implementação:** usar `tsg-flow-task-creator` referenciando esta TechSpec, após a promoção
   acima e confirmação de que F01 está disponível no branch/ambiente onde as tasks serão executadas.
3. **Frontend:** usar `tsg-flow-frontend-techspec-creator` referenciando `api-contract.yaml` e o PRD.
4. **Coordenação com F04:** quando a TechSpec de F04 (Conclusão da Saga) for escrita, apontar este
