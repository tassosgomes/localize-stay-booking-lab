...
- [ ] **Nome do arquivo gerado (`reservationDetail.ts`) e da rota (`/reservations/consultar`)** —
  decisões novas desta TechSpec, sem precedente direto no repositório; confirmar antes de promover
  para `Aprovado`.
...
1. **Revisão:** confirmar as duas decisões em "Questões em Aberto" (nome do arquivo gerado e da rota)
   e promover este documento para `frontend-techspec.md` com status `Aprovado`.
2. **Implementação:** usar `tsg-flow-task-creator` referenciando `frontend-techspec.md`,
...
4. Subir o mock a partir da raiz quando o backend F02 ainda não estiver disponível:
