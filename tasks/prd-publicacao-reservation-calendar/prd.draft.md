...
|---|---|---|
| `reservation_calendar_v1` | Dataset contratado que apresenta uma linha por Reservation em estado terminal, com a Accommodation e o período da estadia, sem expor dados internos de Booking. | `vision.md` §6; `domains/booking/domain.md` F05; proposta desta feature |
| Reservation terminal | Reservation em `confirmada` ou `cancelada`. Esses são os únicos estados publicados por esta versão do calendário. | `domains/booking/domain.md` §3, RN-10 e RN-11; proposta DP-01 |
...
| Ocupação ativa | Período de uma Reservation com estado `confirmada`. Uma linha `cancelada` permanece identificável no dataset, mas não deve ser interpretada como data ocupada. | RN-09; proposta DP-01 |
...
**Rastreabilidade**: RN-01, RN-02, RN-09, RN-10, RN-11; proposta DP-01.
...
**Rastreabilidade**: RN-10, RN-11; proposta DP-02 e proposta DP-03.
...
- **Aprovação de DP-01, DP-02 e DP-03** — responsável: autor do laboratório; prazo desejável: antes
  de converter este draft em `prd.md`; impacto se não resolvido: bloqueia a semântica final de
  elegibilidade, granularidade e atualização do Data Contract.
