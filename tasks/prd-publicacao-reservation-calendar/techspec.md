...
> de view regular e de persistência do instante terminal foram aprovadas e estão registradas
> como ADR-004 e ADR-005, ambas Accepted em 2026-09-13.
...
| docs/adr/adr-004-reservation-calendar-live-view.md | V-01/V-02 | ADR Accepted | tsg-flow-techspec-creator | Registra view regular, DDL versionado, owner de integração e roles somente leitura |
| docs/adr/adr-005-terminal-transition-timestamp.md | EN-01/V-01 | ADR Accepted | tsg-flow-techspec-creator | Registra a persistência do instante terminal na Reservation e o conflito a resolver com F04 |
...
| docs/adr/index.md | EN-01/V-01 | tsg-flow-techspec-creator | Reserva ADR-004/ADR-005 globalmente com status Accepted |
...
| Publicar uma view regular ao vivo | Nova, ADR-004 Accepted | Satisfaz next-successful-read sem refresh e evita componente de runtime na escala Fase 0 |
| Criar DDL em arquivo SQL versionado fora do boot | Nova, ADR-004 Accepted; ownership herdado do baseline | Integration é schema compartilhado e não pode ser migrado pela role de serviço; operador controla DDL |
| Persistir terminal_transition_at na Reservation | Nova, ADR-005 Accepted | Só Booking conhece o instante da transição; leitura/publicação não pode inventá-lo |
...
- **Aprovação registrada:** ADR-004 e ADR-005 foram aceitas e esta TechSpec está promovida para
  `techspec.md`/`Aprovado`, pronta para alimentar o Task Creator. A implementação continua
  dependente do alinhamento técnico de F04 com `TerminalTransitionAt` e da migration correspondente.
...
A decisão aprovada nesta TechSpec mantém a regra no owner Booking com
Reservation.TerminalTransitionAt, migration/constraint e uma única unidade de trabalho. A
aprovação do autor em 2026-09-13 encerra o conflito para efeito da TechSpec. O alinhamento de F04
continua sendo uma dependência de implementação antes da publicação de V-01.

### ADRs Afetadas

- [ADR-004 — Publicação do calendário como view PostgreSQL ao vivo](../../docs/adr/adr-004-reservation-calendar-live-view.md) — promovida para Accepted após a aprovação do autor.
- [ADR-005 — Persistência do instante da transição terminal da Reservation](../../docs/adr/adr-005-terminal-transition-timestamp.md) — promovida para Accepted após a aprovação do autor.
- ADR-001, ADR-002 e ADR-003 são apenas herdadas quando aplicável; não sofrem alteração.

### Registro de aprovação

O autor aprovou este draft em 2026-09-13. A aprovação cobre a escolha de view PostgreSQL regular,
a persistência de terminal_transition_at na Reservation e o handoff necessário com F04. As ADRs
004 e 005 foram promovidas para Accepted e o Handoff desta especificação passou a ser executável
pelo Task Creator, condicionado às dependências técnicas já descritas.
